...
def agreger_donnees():
    ...
    print("📊 Démarrage de l'agrégation des données avec Redis...")
    ...
    try:
        ...
        while redis_client.get_queue_length('queue_normalized') > 0 and records_processed < max_records:
            ...
            if donnee:
                ...
                donnees_aggregees['data'].append(donnee)
                records_processed += 1
                
                # Extraire la source si disponible
                source = donnee.get('source') or donnee.get('origin') or 'unknown'
                if source not in donnees_aggregees['metadata']['sources']:
                    donnees_aggregees['metadata']['sources'].append(source)
        
        donnees_aggregees['metadata']['total_records'] = len(donnees_aggregees['data'])
        ...
        print(f"✅ {len(donnees_aggregees['data'])} données agrégées depuis Redis")
        ...
        # Ajouter des données de fallback pour éviter l'échec complet
    ...
    return f"DATA_AGGREGATED_{len(donnees_aggregees['data'])}"
...
def verifier_etat_redis():
    """Vérification complète de l'état Redis - NOUVELLE FONCTION"""
    try:
        ...
        for nom, queue in queues.items():
            ...
            if length == 0:
                ...
                # Aperçu du premier élément si queue non vide
                if queue == 'queue_normalized' and length > 0:
                    try:
                        preview = redis_client.peek_queue(queue)
                        if preview:
                            preview_str = str(preview)[:100] + "..." if len(str(preview)) > 100 else str(preview)
...
with DAG(
    ...
    schedule_interval=timedelta(minutes=30),  # Toutes les 30 minutes
