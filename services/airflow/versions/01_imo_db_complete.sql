...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'imo_db') THEN
        CREATE DATABASE imo_db;
        RAISE NOTICE 'Base de données imo_db créée';
    ELSE
...
DO $$ BEGIN 
    ...
    RAISE NOTICE 'Tables créées: annonces, caracteristiques, images, dpe, copropriete, conseiller';
    RAISE NOTICE 'Vues créées: annonces_complete, v_stats_type_bien, v_stats_localisation, v_stats_dpe';
    RAISE NOTICE 'Permissions accordées à: imo_user, readonly, grafana';
END $$;