...
class DataExtractor:
    ...
    async def _extract_basic_fields(self, article_locator: Locator, config: Dict) -> Dict[str, Any]:
        ...
        # Code postal - extraction depuis le titre ou localisation
        if 'titre' in fields:
            code_postal_match = re.search(r'\b(\d{5})\b', fields['titre'])
            if code_postal_match:
                fields['code_postal'] = code_postal_match.group(1)
            elif 'localisation' in fields:
                code_postal_match = re.search(r'\b(\d{5})\b', fields['localisation'])
                if code_postal_match:
                    fields['code_postal'] = code_postal_match.group(1)
...
class IADFranceScraper:
    ...
    def __init__(self, config_file: str = "config.json", output_config_file: str = "config_out.json"):
        self.config = self.load_config(config_file)
        self.output_config = self.load_config(output_config_file)
        self.results = []
    ...
    async def fill_search_form(self, localisation: str = "Paris", transaction_type: str = "acheter", property_type: str = "ancien"):
        """Remplit le formulaire de recherche avec les nouveaux paramètres"""
        ...
        try:
            ...
            if transaction_type in search_config['transaction_type']['options']:
                transaction_config = search_config['transaction_type']['options'][transaction_type]
                await self.page.click(transaction_config['css_selector'], force=True)
                await self.page.wait_for_timeout(1000)
                print(f"✓ Type de transaction sélectionné: {transaction_type.capitalize()}")
                
                # Gestion spéciale pour la location - cliquer sur "Locataire"
                if transaction_type == "louer":
                    await self._handle_rental_search()
                
            else:
                print(f"⚠️ Type de transaction '{transaction_type}' non trouvé, utilisation par défaut: acheter")
                transaction_config = search_config['transaction_type']['options']['acheter']
                await self.page.click(transaction_config['css_selector'], force=True)
                await self.page.wait_for_timeout(1000)
            
            # Sélectionner le type de bien
            if property_type in search_config['type_bien']['options']:
                type_bien_config = search_config['type_bien']['options'][property_type]
                await self.page.click(type_bien_config['css_selector'], force=True)
                await self.page.wait_for_timeout(1000)
                print(f"✓ Type de bien sélectionné: {property_type.capitalize()}")
            else:
                print(f"⚠️ Type de bien '{property_type}' non trouvé, utilisation par défaut: ancien")
    ...
    async def _handle_rental_search(self):
        """Gère la recherche de biens à louer - clique sur 'Locataire'"""
        try:
            print("🏠 Recherche de location - recherche du bouton 'Locataire'...")
            
            # Attendre un peu pour que l'interface se mette à jour
            await self.page.wait_for_timeout(2000)
            
            # Essayer différents sélecteurs pour trouver le bouton "Locataire"
            locataire_selectors = [
                "button:has-text('Locataire')",
                "label:has-text('Locataire')",
                "[class*='locataire']",
                "input[value*='locataire']"
            ]
            
            for selector in locataire_selectors:
                try:
                    locataire_btn = self.page.locator(selector)
                    if await locataire_btn.count() > 0 and await locataire_btn.is_visible():
                        await locataire_btn.click(force=True)
                        await self.page.wait_for_timeout(1000)
                        print("✓ Bouton 'Locataire' cliqué")
                        return
                except Exception:
                    continue
            
            print("⚠️ Bouton 'Locataire' non trouvé, continuation sans sélection spécifique")
            
        except Exception as e:
            print(f"⚠️ Erreur lors de la gestion de la recherche location: {e}")
    ...
    async def scrape(self, localisation: str = "Paris", transaction_type: str = "acheter", 
                    property_type: str = "ancien", max_biens: int = None, max_pages: int = None):
        """Fonction principale de scraping avec les nouveaux paramètres"""
        ...
        try:
            ...
            await self.fill_search_form(localisation, transaction_type, property_type)
            ...
            print(f"📍 Paramètres utilisés: {localisation}, {transaction_type}, {property_type}")
    ...
    async def save_results_json(self, data: List[Dict[str, Any]]) -> str:
        """Sauvegarde les résultats dans un fichier JSON selon la structure config_out.json"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Sauvegarde dans le répertoire courant ou le sous-répertoire results
        results_dir = "results"
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)
            print(f"📁 Création du répertoire: {results_dir}")
        
        filename = f"{results_dir}/iad_annonces_{timestamp}.json"
        
        # Utiliser la structure de sortie définie dans config_out.json
        output_structure = self.output_config.get('output_structure', {})
        output_fields = output_structure.get('fields', [])
        
        # Formater les données selon la structure de sortie
        formatted_data = []
        for item in data:
            formatted_item = {}
            for field in output_fields:
                # Gérer les champs spéciaux
                if field == 'date_extraction':
                    formatted_item[field] = datetime.now().isoformat()
                elif field == 'type_transaction':
                    formatted_item[field] = getattr(self, 'current_transaction_type', 'acheter')
                elif field == 'type_bien':
                    formatted_item[field] = getattr(self, 'current_property_type', 'ancien')
                elif field == 'code_postal':
                    # Extraire le code postal si non présent
                    if 'code_postal' not in item:
                        code_postal_match = re.search(r'\b(\d{5})\b', item.get('localisation', '') or item.get('titre', ''))
                        if code_postal_match:
                            formatted_item[field] = code_postal_match.group(1)
                        else:
...
async def main():
    """Fonction principale d'exécution avec les nouveaux paramètres"""
    ...
    parser.add_argument('--transaction', type=str, default=os.getenv('TRANSACTION_TYPE', 'acheter'),
                       choices=['acheter', 'louer', 'vendre'],
                       help='Type de transaction: acheter, louer, vendre')
    parser.add_argument('--bien', type=str, default=os.getenv('PROPERTY_TYPE', 'ancien'),
                       choices=['ancien', 'neuf', 'prestige', 'international', 'terrain', 'entreprises_commerces', 'immeuble'],
                       help='Type de bien: ancien, neuf, prestige, international, terrain, entreprises_commerces, immeuble')
    ...
    scraper = IADFranceScraper("config.json", "config_out.json")
    
    # Stocker les paramètres courants pour la structure de sortie
    scraper.current_transaction_type = args.transaction
    scraper.current_property_type = args.bien
    
    print("🚀 Démarrage du scraping IAD France...")
    print(f"📍 Localisation: {args.localisation}")
    print(f"💰 Type de transaction: {args.transaction}")
    print(f"🏠 Type de bien: {args.bien}")
